    ...
    public class SellerLoginController : Controller
    {
        ...
        public ActionResult Index(UserLoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    if (ValidateMobileIsExists(model.MobileNumber,"V"))
                    {
                        Session["UserType"] = "V";
                    ...
                    else
                    {
                        ...
                        SaveUserAndOTP(model.MobileNumber, "V");
                        model.OTPSent = true;
                    }
        ...
        private void UpdateOTP(string mobileNumber)
        {
            ...
            using (SqlConnection con = new SqlConnection(_connectionString))
            {
                ...
                cmd.Parameters.Add("@USER_TYPE", SqlDbType.VarChar).Value = "V";
        ...
        private bool ValidateOTP(string mobileNumber, string otp)
        {
            ...
            using (SqlConnection con = new SqlConnection(_connectionString))
            {
                string query = "SELECT COUNT(1) FROM TBL_USER_LOGIN_MASTER WHERE MOBILE_NO = @MobileNumber AND OTP = @OTP AND USER_TYPE = @USER_TYPE";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.Parameters.AddWithValue("@MobileNumber", mobileNumber);
